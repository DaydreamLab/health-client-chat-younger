...
test('guest home page loads', async ({ page, goto }) => {
  ...
  await expect(page.getByTestId('plan-basicCare')).toBeVisible()
  await expect(page.getByTestId('plan-fullTune')).toBeVisible()
...
test('plan CTA opens chat and AI can reply', async ({ page, goto }) => {
  ...
  await page.getByTestId('plan-basicCare').click()
